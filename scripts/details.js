// Getting elements from html file using id's
...
fetch(`https://superheroapi.com/api.php/1166441817229936/${id}`)
    .then(res => res.json())
...
function showDetails(data) {
    ...
    favList.includes(id)
        ?
        fav.innerHTML = '<i id="fav-icon" class="fas fa-star fa-4x"></i>'
        :
        fav.innerHTML = '<i id="fav-icon" class="far fa-star fa-4x"></i>'
...
fav.onclick = function () {
    ...
    //Else remove from Favourites
        ...
        favList.splice(i, 1);
        //Changing Fav-icon class
        fav.innerHTML = '<i id="fav-icon" class="far fa-star"></i>';
        showSnackbar(false);
    }
